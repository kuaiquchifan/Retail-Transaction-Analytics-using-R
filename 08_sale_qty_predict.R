...
monthly_qty <- combine_data %>% 
  ...
  summarise(
    monthly_qty = sum(quantity, na.rm = TRUE),
    .groups = "drop"
  ) %>% 
  arrange(month)
...
full_months <- tibble(
  month = seq.Date(
    from = min(monthly_qty$month),
    to = max(monthly_qty$month),
    by = "month"
  )
)

monthly_qty <- full_months %>%
  left_join(monthly_qty, by = "month") %>%
  mutate(
    monthly_qty = replace_na(monthly_qty, 0)
  )

cat("\n月销售数量数据预览:\n")
print(head(monthly_qty, 12))
...
monthly_qty <- monthly_qty %>%
  mutate(
    ...
    monthly_qty = monthly_qty * trend_factor * seasonal_factor * promo_factor
  )
...
# 3. 构造时间序列对象
# ----------------------------------------------------------
sales_values <- monthly_qty$monthly_qty
start_year <- year(min(monthly_qty$month))
start_month <- month(min(monthly_qty$month))
...
train_ts <- ts(
  ...
  start = c(
    year(monthly_qty$month[train_start_idx]),
    month(monthly_qty$month[train_start_idx])
  ),
  frequency = 12
)
...
test_ts <- ts(
  ...
  start = c(
    year(monthly_qty$month[train_end_idx + 1]),
    month(monthly_qty$month[train_end_idx + 1])
  ),
  frequency = 12
)
...
make_ts_features <- function(df) {
  df %>%
    ...
    mutate(
      ...
      lag_1  = dplyr::lag(monthly_qty, 1),
      lag_3  = dplyr::lag(monthly_qty, 3),
      lag_6  = dplyr::lag(monthly_qty, 6),
      lag_12 = dplyr::lag(monthly_qty, 12),

      ma_3 = zoo::rollmean(monthly_qty, k = 3, fill = NA, align = "right"),
      ma_6 = zoo::rollmean(monthly_qty, k = 6, fill = NA, align = "right")
    )
}
...
ml_features <- monthly_qty %>%
  select(month, monthly_qty) %>%
  make_ts_features() %>%
  filter(complete.cases(.))

stopifnot(is.data.frame(monthly_qty))
cat("\n=== 调试：monthly_qty 类型检查 ===\n")
print(class(monthly_qty))

cat("\n=== 调试：monthly_qty 结构 ===\n")
str(monthly_qty)

train_months <- monthly_qty$month[train_idx]
test_months  <- monthly_qty$month[test_idx]
...
# ML 递归多步预测
predict_ml_recursive <- function(model, model_name, monthly_qty_full, 
                                 feature_cols, h = 6) { 
  history <- monthly_qty_full %>% 
    select(month, monthly_qty)
  ...
  for (i in seq_len(h)) {
    ...
    temp_df <- history %>%
      bind_rows(tibble(month = next_month, monthly_qty = NA_real_)) %>%
      make_ts_features()
    ...
    history <- history %>%
      bind_rows(tibble(month = next_month, monthly_qty = pred))
  }
...
# 6.1 Prophet
prophet_df <- monthly_qty %>%
  select(ds = month, y = monthly_qty)
...
write.csv(model_metrics, file.path(output_dir, "08_monthly_sales_qty_model_comparison.csv"), row.names = FALSE)
write.csv(monthly_qty, file.path(output_dir, "08_monthly_sales_qty_series.csv"), row.names = FALSE)
saveRDS(all_models, file.path(output_dir, "08_monthly_sales_qty_forecast_models.rds"))

# ----------------------------------------------------------
# 11. 可视化
# ----------------------------------------------------------
test_dates <- monthly_qty$month[test_idx]
...
actual_data <- monthly_qty %>%
  select(month, sales = monthly_qty)
...
forecast_plot <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste(
      "Training:",
      format(min(monthly_qty$month[train_idx]), "%Y-%m"),
      "~",
      format(max(monthly_qty$month[train_idx]), "%Y-%m"),
      "| Test:",
...
ggsave(
  filename = file.path(
    output_dir,
    "08_monthly_sales_qty_all_models_forecast.png"
  ),
  plot = forecast_plot,
  width = 14,
  height = 8,
...
cat(
  "\n所有模型预测图已保存:",
  file.path(output_dir, "08_monthly_sales_qty_all_models_forecast.png"),
  "\n"
)
...
# 输出最佳模型预测图
best_model_dates <- monthly_qty$month[test_idx]

best_model_forecast <- tibble(
  month = best_model_dates,
  sales = as.numeric(best_model_fc$mean),
  Model = best_model_name
)

best_model_plot <- ggplot() +
  geom_line(
    data = actual_data,
    aes(x = month, y = sales),
    linewidth = 1,
  ...
  labs(
    title = paste("Best Model Forecast:", best_model_name),
    subtitle = paste(
      "Training:",
      format(min(monthly_qty$month[train_idx]), "%Y-%m"),
      "~",
...
ggsave(
  filename = file.path(output_dir, "08_monthly_best_sales_qty_forecast.png"),
...
cat(
  "\n最佳模型单图已保存:",
  file.path(output_dir, "08_monthly_best_sales_qty_forecast.png"),
  "\n"
)
...
# ----------------------------------------------------------
future_horizon <- 6
last_month <- tail(monthly_qty$month, 1)
...
write.csv(
  future_forecast_tbl,
  file.path(output_dir, "08_future_6_month_sales_qty_forecast.csv"),
  row.names = FALSE
)
...
cat("============================= \n")
cat("模型比较文件: data/processed-v2/08_monthly_sales_model_qty_comparison.csv\n")
cat("月销售序列:   data/processed-v2/08_monthly_sales_qty_series.csv\n")
cat("预测图:       data/processed-v2/08_monthly_sales_qty_all_models_forecast.png\n")
cat("未来6个月预测: data/processed-v2/08_future_6_month_sales_qty_forecast.csv\n")
